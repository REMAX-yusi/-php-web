...
function imgUpload($file)
{
    ...
    //上传图像名称
    $img = strtolower(pathinfo($file['name'], PATHINFO_FILENAME));//不含后缀
    $imge = $img . '.' . $ext;//加上后缀
    //上传文件夹
    $fileDir = '/static/file/' . $img . '/';

    //检查上传目录是否存在
    if (!is_dir($fileDir)) {

        mkdir($fileDir, 0755, true); //递归创建目录
    }


    //物理地址
    $imgPath = $fileDir . $imge;
    //url地址
    $imgUrl = 'http://localhost/web' . $fileDir . $imge;
    //操作失败 查看上传目录的权限
